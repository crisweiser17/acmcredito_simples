...
class Migrator {
  public static function run(): void {
    ...
    $pdo = Connection::get();
    $sql = [];
    $sql[] = "CREATE TABLE IF NOT EXISTS config (id INT PRIMARY KEY AUTO_INCREMENT, chave VARCHAR(100) UNIQUE NOT NULL, valor VARCHAR(255) NOT NULL, descricao TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)";
    $sql[] = "CREATE TABLE IF NOT EXISTS clients (id INT PRIMARY KEY AUTO_INCREMENT, nome VARCHAR(255) NOT NULL, cpf VARCHAR(14) UNIQUE NOT NULL, data_nascimento DATE NOT NULL, email VARCHAR(255), telefone VARCHAR(20), cep VARCHAR(9), endereco VARCHAR(255), numero VARCHAR(20), complemento VARCHAR(100), bairro VARCHAR(100), cidade VARCHAR(100), estado VARCHAR(2), ocupacao VARCHAR(100), tempo_trabalho VARCHAR(50), renda_mensal DECIMAL(10,2), doc_holerites JSON, doc_cnh_frente VARCHAR(255), doc_cnh_verso VARCHAR(255), doc_selfie VARCHAR(255), cnh_arquivo_unico BOOLEAN DEFAULT 0, prova_vida_status ENUM('pendente','aprovado','reprovado') DEFAULT 'pendente', prova_vida_data DATETIME, prova_vida_user_id INT, cpf_check_status ENUM('pendente','aprovado','reprovado') DEFAULT 'pendente', cpf_check_data DATETIME, cpf_check_user_id INT, observacoes TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, deleted_at TIMESTAMP NULL, INDEX idx_cpf (cpf), INDEX idx_nome (nome))";
